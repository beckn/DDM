...
A developer toolkit for building and testing applications on the **beckn DDM (Decentralised Data Marketplace) testnet** using the Rain Probability use case. This devkit provides a pre-configured adapter stack and ready-to-use Postman collections to help you get started quickly.
...
The Rain Probability Devkit enables developers to simulate and test decentralised data exchange for rain probability data over the beckn protocol. It bundles a Docker-based adapter setup and Postman collections so you can spin up a local environment and begin making beckn-compliant API calls within minutes.
...
```bash
git clone https://github.com/beckn/DDM.git
cd DDM/testnet/rain-probability-devkit/install
```
...
The `postman/` directory contains pre-built collections for testing the Rain Probability APIs.
...
4. Select the collection file (`.json`) and click **Open**.
...
The collections are ordered to reflect a typical beckn transaction flow (e.g., `search` → `on_search` → `select` → `on_select`, etc.). Run them in sequence for an end-to-end test.
